...
public class UserController {
	...
	public String kakaoCallback(@RequestParam String code) {
		...
		// 우리 blog 서버에 로그인 처리해ㅔ서 세션 처리 .... 진행을 해야 한다.
		
		// User Object 만들기
		// 1. 회원가입 -- 자동으로 --> 우리 DB에 넣을 예정
		// 2. 소셜 로그인 사용자에게 우리 사이트에 이미 회원 가입된 사용자라면 DB 저장 x
		KakaoAccount account = kakaoDataResponse.getBody().kakaoAccount;
		
		// 회원가입할 때 필요한 사항 - username, email, password
		// insert2 eat_"a"
		User kakaoUser = User
				.builder()
		...
		// 여기서는 user id를 당연히 사용 못한다. 그럼 username으로 검색하는 기능을 만들어 주어야 한다.
		User originUser = userService.searchUserName(kakaoUser.getUsername());
		
		if(originUser.getUsername() == null) {
			System.out.println("신규 회원이기 때문에 회원 가입을 진행");
			userService.saveUser(kakaoUser);
		}
		
		// 신규로 회원 가입이든, 기존에 한번 가입 했던 유저이든 무조건 소셜 로그인이면 세션 생성해 주어야 한다.
