...
<div class="container">
	...
	<ul class="pagination justify-content-center">
		...
		<li class="page-item ${boards.first ? isDisabled : isNotDisabled}"><a class="page-link" href="/board/search/?q=${q}&page=${boards.number-1}">Previous</a>
		</li>
		<!-- 총 페이지가 몇개인지 알아야 한다. -->
		...
		<c:forEach var="num" items="${pageNumbers}">
			<c:choose>
				<c:when test="${nowPage eq num}">
					<li class="page-item active"><a class="page-link" href="/board/search?q=${q}&page=${num - 1}">${num}</a></li>
				</c:when>
				<c:otherwise>
					<li class="page-item"><a class="page-link" href="/board/search?q=${q}&page=${num - 1}">${num}</a></li>
				</c:otherwise>
			</c:choose>
